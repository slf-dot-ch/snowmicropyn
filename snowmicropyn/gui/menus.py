...
# uses wx framework to guarantee cross platform compatibility
###########################################################
...
class PlotOptions(wx.Dialog):
    def __init__(self, parent=None, id=-1, title="Plot Options"):
        ...
        self.optionColor = wx.ComboBox(self, -1, pos=(265, 50), size=(150, -1), choices=colors, style=wx.CB_READONLY,
                                       value=self.color)

        wx.StaticText(self, -1, 'Style', (265, 80))
        self.optionStyle = wx.ComboBox(self, -1, pos=(265, 100), size=(150, -1), choices=styles, style=wx.CB_READONLY,
                                       value=self.style)

        wx.StaticText(self, -1, 'Width', (265, 140))
        self.optionWidth = wx.SpinCtrl(self, -1, name="Width", initial=1, min=0.5, max=10, pos=(325, 135),
                                       size=(50, -1))
...
class GraphOptions(wx.Dialog):
    ...
    def __init__(self, parent=None, extended=True):
        """Constructor"""
        wx.Dialog.__init__(self, parent, wx.ID_ANY, "Graph Options", size=(600, 450))
        ...
        self.xmin = FloatSpin(notebook.tabAxis, -1, value=self.xlim[0], min_val=-100.00, max_val=1800.00, increment=1,
                              digits=3)
        label_xmin = wx.StaticText(notebook.tabAxis, -1, 'X min')
        self.ymin = FloatSpin(notebook.tabAxis, -1, value=self.ylim[0], min_val=-100.00, max_val=1800.00, increment=1,
                              digits=3)
        label_ymin = wx.StaticText(notebook.tabAxis, -1, 'Y min')
        self.xmax = FloatSpin(notebook.tabAxis, -1, value=self.xlim[1], min_val=-100.00, max_val=1800.00, increment=1,
                              digits=3)
        label_xmax = wx.StaticText(notebook.tabAxis, -1, 'X max')
        self.ymax = FloatSpin(notebook.tabAxis, -1, value=self.ylim[1], min_val=-100.00, max_val=1800.00, increment=1,
                              digits=3)
        ...
        self.xticks_ctrl = wx.SpinCtrl(notebook.tabAxis, -1, size=(100, -1), min=1, max=100, initial=self.xticks,
                                       name="X ticks")
        label_yticks = wx.StaticText(notebook.tabAxis, -1, 'Y ticks')
        self.yticks_ctrl = wx.SpinCtrl(notebook.tabAxis, -1, size=(100, -1), min=1, max=100, initial=self.yticks,
                                       name="Y ticks")
        ...
        self.sampling_ctrl = wx.SpinCtrl(notebook.tabGraph, -1, size=(100, -1), min=0, max=1000, initial=self.sampling,
                                         name="Sampling")
        label_color = wx.StaticText(notebook.tabGraph, -1, 'Line Color')
        self.color_ctrl = wx.ComboBox(notebook.tabGraph, -1, size=(150, -1), choices=colors, style=wx.CB_READONLY,
                                      value=self.color)
        label_style = wx.StaticText(notebook.tabGraph, -1, 'Line Style')
        self.style_ctrl = wx.ComboBox(notebook.tabGraph, -1, size=(150, -1), choices=styles, style=wx.CB_READONLY,
                                      value=self.style)
        label_width = wx.StaticText(notebook.tabGraph, -1, 'Line Width')
        self.width_ctrl = wx.SpinCtrl(notebook.tabGraph, -1, name="Width", initial=1, min=0.5, max=10, pos=(325, 135),
                                      size=(50, -1))
        ...
        self.grad_sampling_ctrl = wx.SpinCtrl(notebook.tabGraph, -1, size=(100, -1), min=0, max=1000,
                                              initial=self.grad_sampling, name="Sampling")
        label_grad_color = wx.StaticText(notebook.tabGraph, -1, 'Line Color')
        self.grad_color_ctrl = wx.ComboBox(notebook.tabGraph, -1, size=(150, -1), choices=colors, style=wx.CB_READONLY,
                                           value=self.grad_color)
        label_grad_style = wx.StaticText(notebook.tabGraph, -1, 'Line Style')
        self.grad_style_ctrl = wx.ComboBox(notebook.tabGraph, -1, size=(150, -1), choices=styles, style=wx.CB_READONLY,
                                           value=self.grad_style)
        label_grad_width = wx.StaticText(notebook.tabGraph, -1, 'Line Width')
        self.grad_width_ctrl = wx.SpinCtrl(notebook.tabGraph, -1, name="Width", initial=1, min=0.5, max=10,
                                           pos=(325, 135), size=(50, -1))

        # median
        label_median = wx.StaticText(notebook.tabGraph, -1, "Median")
        label_median_sampling = wx.StaticText(notebook.tabGraph, -1, 'Window')
        self.median_sampling_ctrl = wx.SpinCtrl(notebook.tabGraph, -1, size=(100, -1), min=0, max=1000,
                                                initial=self.median_sampling, name="Sampling")
        label_median_color = wx.StaticText(notebook.tabGraph, -1, 'Line Color')
        self.median_color_ctrl = wx.ComboBox(notebook.tabGraph, -1, size=(150, -1), choices=colors,
                                             style=wx.CB_READONLY, value=self.median_color)
        label_median_style = wx.StaticText(notebook.tabGraph, -1, 'Line Style')
        self.median_style_ctrl = wx.ComboBox(notebook.tabGraph, -1, size=(150, -1), choices=styles,
                                             style=wx.CB_READONLY, value=self.median_style)
        label_median_width = wx.StaticText(notebook.tabGraph, -1, 'Line Width')
        self.median_width_ctrl = wx.SpinCtrl(notebook.tabGraph, -1, name="Width", initial=1, min=0.5, max=10,
                                             pos=(325, 135), size=(50, -1))
...
import os
from snowmicropyn.analysis import mathematics as calc
...
class SuperPosition(wx.Frame):
    ...
    def createPanel(self):
        ...
        self.list = ulc.UltimateListCtrl(self.panelPlot,
                                         agwStyle=wx.LC_REPORT | wx.LC_VRULES | wx.LC_HRULES | wx.LC_SINGLE_SEL | ulc.ULC_HAS_VARIABLE_ROW_HEIGHT)
    ...
    def OnSubtract(self, e):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for entry in files:
                ...
                if xsub_min >= xref_min:
                    xmin = xsub_min
                else:
                    xmin = xref_min
                if xsub_max >= xref_max:
                    xmax = xref_max
                else:
                    xmax = xsub_max
                ...
                self.axes.plot(x_new, y_new,
                               ...
                               label=basename(entry.filename) + " - " + basename(
                                   ref.filename) + ", rsme = %0.3g" % rsme)
    ...
    def adaptData(self, pnt):
        x_corrected = (snowmicropyn.analysis.downsample(pnt.data[:, 0],
                                                        pnt.smooth.GetValue()) + pnt.dx.GetValue()) * pnt.stretch.GetValue()
        y_corrected = snowmicropyn.analysis.downsample(pnt.data[:, 1], pnt.smooth.GetValue()) + pnt.dy.GetValue()

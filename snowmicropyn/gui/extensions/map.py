...
import os
import string
from snowmicropyn.gui.menus import CheckListCtrl
...
class Map(wx.Frame):
    ...
    def isListChecked(self):
        if [index for index in range(self.list.ItemCount) if self.list.IsChecked(index)] == []:
    ...
    def GetLabel(self):
        ...
        try:
            ...
            for entry in range(len(self.files)):
                if self.list.IsChecked(entry):
                    label.append("&markers=size:mid|color:red%7Clabel:" + self.label[entry] + "%7C" + str(
                        self.files[entry].header["Latitude"]) + "," + str(self.files[entry].header["Longitude"]))
        except:
    ...
    def OnExport(self, e):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for i in range(len(self.files)):
                line = "%s\t%f\t%f\t%.2f\n" % (
                    os.path.basename(self.files[i].filename), self.files[i].header["Longitude"],
                    self.files[i].header["Latitude"], self.files[i].header["PDOP"])
                f.write(line)
            f.close()

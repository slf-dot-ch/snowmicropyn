...
def shotnoise_from_evalSMP(dz,f_z,A_cone=19.6):
    ...
    detrended_f_z=signal.detrend(f_z-c1,type='linear')
...
def getSNParams(file, window=2.5,overlap=50):
    ...
    while x0 + window <= x[-1]:
        ...
        shotnoise_data=shotnoise_from_evalSMP(dz,y[start:end])
...
def calculate_density_and_ssa_proksch(medianF,L):
    #Parametrizations of density and SSA presented in Proksch 2015 JGR
    a=420.47;b=102.47;c=-121.15;d=-169.96
    rho= a + b*numpy.log(medianF) + c*numpy.log(medianF)*L + d*L
    e=0.1312;f=0.3548;g=0.0291
    lc= e + f*L + g*numpy.log(medianF)
